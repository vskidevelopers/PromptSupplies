...
export default function LeftSideProjectCard({ project }) {
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-28">
      <div className="border border-[#FDB715] transform transition-all duration-300 hover:bg-[#FDB715] h-full ">
        <img
          className="rounded  cursor-pointer ease-linear hover:translate-x-4 hover:translate-y-4  hover:z-10 transition-all duration-200"
          src={project?.imageUrl}
          alt={`Screenshot of ${project?.title} website website`}
        />
      </div>

      <div className="py-4 ml-4 md:p-0 bg-inherit rounded-md">
