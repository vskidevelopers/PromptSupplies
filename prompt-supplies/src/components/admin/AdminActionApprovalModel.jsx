...
export default function AdminActionApprovalModel({ message, item, action }) {
  ...
  const handleClick = (actionParameter) => {
    ...
    if (actionParameter === "approve") {
      try {
        ...
        handleApproveServiceItem(item.id);
        setShowModal(false);
      } catch (err) {
        console.log("the following error ocuured >>", err);
      }
    } else if (actionParameter === "popular") {
      try {
        console.log("3. function to run is >>", actionParameter);
        console.log("Popular Item#", item.id);
        handleMakePopular(item.id);
        setShowModal(false);
      } catch (err) {
        console.log("the following error ocuured >>", err);
      }
      ...
      try {
        ...
        handleMakeFeatured(item.id);
        setShowModal(false);
      } catch (err) {
        console.log("the following error ocuured >>", err);
      }
    } else if (actionParameter === "sale") {
      try {
        console.log("3. function to run is >>", actionParameter);
        console.log("Sale Item#", item.id);
        handleMakeSale(item.id);
        setShowModal(false);
      } catch (err) {
        console.log("the following error ocuured >>", err);
      }
      ...
      try {
        console.log("3. function to run is >>", actionParameter);
        console.log("Offer Item#", item.id);
        handleMakeOffer(item.id);
        setShowModal(false);
      } catch (err) {
        console.log("the following error ocuured >>", err);
      }
